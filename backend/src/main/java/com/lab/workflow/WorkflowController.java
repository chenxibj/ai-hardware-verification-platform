...
public class WorkflowController {
    ...
    public ApiResponse<?> create(@RequestBody Map<String, Object> body) {
        return ApiResponse.success(Map.of("id", UUID.randomUUID().toString(), "status", "DRAFT"));
    }

    @GetMapping("/{id}")
    public ApiResponse<?> get(@PathVariable String id) {
        return ApiResponse.success(Map.of("id", id, "name", "示例流程", "status", "DRAFT", "nodes", List.of()));
    }
