...
> - v2.3 (2026-04-15): **产品决策变更** — 评分/打分/芯片对比（vs L40S 百分比、雷达图、场景推荐等）从单个评测报告移到独立的「评测报告对比」功能；单个报告仅展示原始测试数据
...
| ★ 单报告 vs 对比 | 评分/对比在报告内 | **评分/对比移到独立对比功能** | v2.3: 单报告只展示原始数据，评分/雷达图/vs 百分比在「报告对比」中展示 |
...
> **★ v2.3 变更（2026-04-15）：** 八维评价体系保留，但 vs L40S 百分比计算和雷达图展示 **移到「评测报告对比」功能中**，不在单个评测报告中展示。单个报告仅展示各维度的原始测试数据。
...
│   ├── 6.5 并发性能与 SLA 达标率
│   └── 6.6 多模态推理（可选）
├── ~~7. 综合评价（八维 vs L40S 百分比）~~  ← ★ v2.3: 移到「评测报告对比」功能
│   ├── ~~7.1 综合能力概览~~
│   ├── ~~7.2 分段详细结论~~
│   ├── ~~7.3 适用场景评估~~
│   └── ~~7.4 瓶颈分析与优化建议~~  → 保留瓶颈分析在单报告中（基于原始数据）
├── 8. 评测环境与方法论
└── 附录
```
...
> **★ v2.3 变更（2026-04-15）：** 竞品规格对比表移到「评测报告对比」功能中展示。单个报告仅展示被测芯片自身规格卡片（2.1）。
...
> **★ v2.3 变更（2026-04-15）：** 单个报告仅展示被测芯片数据。L40S 列和 vs L40S 列移到对比功能。
...
> **★ v2.3 变更（2026-04-15）：** 单个报告保留被测芯片 busbw 和延迟列。L40S 列和 vs L40S 列移到对比功能。
...
```json
{
  ...
  "baseline_comparison": {   // ★ v2.3: 此字段仅在对比功能中使用
...
> **★ v2.3 变更（2026-04-15）：** 训练性能摘要在单个报告中仅展示 **原始吞吐数据**（Tokens/s、Samples/s 等）。vs L40S 百分比列移到「评测报告对比」功能中展示。

> **基准芯片: NVIDIA L40S = 100%**，所有性能值均以 L40S 实测值为基准换算百分比。 *(★ v2.3: 此基准百分比呈现方式仅在对比功能中使用)*
...
> **★ v2.3 变更（2026-04-15）：** 单个报告表格仅保留「被测芯片」列（原始吞吐数据+柱状图）。「L40S (基准)」和「vs L40S」列移到对比功能中展示。
...
**可视化：** 柱状图（被测芯片原始吞吐数据）*(★ v2.3: 分组对比柱状图和性能比值热力图移到对比功能)*
...
> **★ v2.3 变更（2026-04-15）：** 单个报告保留被测芯片的扩展性数据。L40S 扩展效率列移到对比功能。
...
**可视化:** 折线图（虚线=理想线性，实线=被测芯片实际）*(★ v2.3: L40S 对比折线在对比功能中展示)*
...
> **★ v2.3 变更（2026-04-15）：** 推理性能摘要在单个报告中仅展示 **原始性能数据**（TGS、延迟等）。vs L40S 百分比及场景对比移到「评测报告对比」功能中展示。

> **基准芯片: NVIDIA L40S = 100%** *(★ v2.3: 此基准百分比呈现方式仅在对比功能中使用)*
...
> **★ v2.3 变更（2026-04-15）：** 单个报告仅保留「被测芯片 Output TGS」列。L40S 列、vs L40S 列、雷达图移到对比功能。
...
**可视化：** 柱状图（被测芯片各配置 TGS）*(★ v2.3: 热力图、雷达图、分组对比移到对比功能)*
...
> **★ v2.3 变更（2026-04-15）：** 单个报告仅展示被测芯片的 Decode TGS 和 Prefill TGS。L40S 列和 vs L40S 列移到对比功能。
...
> **★ v2.3 变更（2026-04-15）：** 单个报告保留被测芯片 QPS、P99 延迟、SLA 达标率。L40S QPS 和 vs L40S 列移到对比功能。
...
> **★ v2.3 变更（2026-04-15）：** 整个 Section 7（综合评价）从单个评测报告中移除，移到独立的「评测报告对比」功能中实现。单个报告新增「瓶颈分析」Section，基于原始测试数据自动定位最慢算子等问题。以下 7.1-7.4 的设计内容保留，作为对比功能的设计参考。

**7.1 综合能力概览（基于 L40S 基准）** *(★ v2.3: 在对比功能中展示)*
...
**★ 场景加权视图（展示层增强，不改评分算法）** *(★ v2.3: 在对比功能中展示)*
...
> **★ v2.3 变更（2026-04-15）：** 报告列表移除「vs L40S 综合比值」列。列表仅显示：编号/计划/完成率/状态/时间/操作。综合比值在「评测报告对比」功能中查看。

### US-2.2: 多报告对比分析 ★ v2.3 增强（评分/对比功能的新归属）

> **★ v2.3 变更（2026-04-15）：** 此功能成为评分/对比的唯一入口。原单个报告中的 vs L40S 百分比、八维雷达图、综合评价、场景推荐等内容全部移到此对比功能中。用户选择 2+ 份报告后，才计算 vs 百分比、展示雷达图。
...
- ★ v2.3 新增：综合评价（八维 vs L40S 百分比、分段结论、场景推荐）在此展示
- 导出：[📥 对比报告PDF]
...
### 9.4 报告页 ★ v2.2 增强 / v2.3 更新

> **★ v2.3 变更（2026-04-15）：** 单个报告页移除综合评价/雷达图/芯片规格对比，仅展示原始测试数据。新结构如下：

报告头(芯片名/评测计划/完成率/导出按钮) → 执行摘要(覆盖率/通过率等统计) → 算子性能(延迟/吞吐原始数据表格+柱状图) → 训练性能(原始吞吐数据) → 推理性能(原始 TGS/延迟数据) → 瓶颈分析(基于原始数据的最慢算子等) → 评测环境

> ~~v2.2 旧结构: 报告头 → 执行摘要 → 芯片规格概览 → 芯片基础性能(含 NCCL 通信) → 算子评测 → 训练性能评测 → 推理性能评测(含 Decode/Prefill) → 综合评价(八维 vs L40S 雷达图) → 瓶颈分析 → 评测环境~~
...
> **★ v2.3 变更（2026-04-15）：** 本附录中的评分算法（vs L40S 百分比计算、综合比值、颜色规则等）**仅适用于「评测报告对比」功能**，不在单个评测报告中使用。单个报告仅展示原始测试数据，不计算评分。
...
*v2.3 核心变更：评分/打分/芯片对比（vs L40S 百分比、八维雷达图、综合评价、场景推荐）从单个评测报告移到独立的「评测报告对比」功能（US-2.2）。单个报告仅展示原始测试数据（延迟、吞吐、P95/P99、通过率等），对比功能中才计算 vs 百分比和展示雷达图。*
