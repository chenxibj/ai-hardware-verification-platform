...
- **🔴 需求必须先出设计文档（2026-04-19 铁律）** — 当消息中出现"需求"关键字时，**必须先在 GitHub 项目 repo 里提交新的设计文档或更新现有设计文档**（docs/ 目录），再拆 issue 再开发。流程：
  1. 理解需求，不清楚的地方**先和 chenxi 澄清确认**
  2. 写设计文档（背景、方案、数据模型、API 设计、交互流程、验收标准）
  3. commit 到 repo 的 `docs/` 目录，push 到 GitHub
  4. 和 chenxi 确认设计方案
  5. 确认后再拆 issue、开发
...
- **🔴🔴 TDD 是开发铁律（2026-04-11 重大升级，04-16 chenxi 再次确认必须落实）** ——
  ...
  - **🔴 04-16 执行反思：** 之前写了铁律但执行打折扣 — #459 十个 phase 零测试、大部分 commit 先实现后补测试。从今天起 sub-agent task prompt 强制包含 TDD 三步结构（见 docs/dev-standards.md），CI 红不准推新功能
  - **🔴🔴 04-17 chenxi 强调：CI 测试集必须随 bug 修复持续更新** — 每个 bug fix 必须附带对应测试用例一起进 CI。测试集不是静态的，是活的回归保护网。修复和测试必须同一个 commit/PR，不允许分开。
  - **完整规范文档：** `docs/dev-standards.md`（TDD 流程 + CI/CD 准则 + sub-agent 模板）
...
- **Agent timeout ≠ 任务未完成（04-17）：** 两个 agent 超时，但代码修复+构建+部署都已完成，超时原因是验证阶段耗时。看到 timeout 先检查代码和部署状态，不要盲目重跑。
...
- **✅ 04-17 验证：模块隔离并行有效** — 3 个 agent 严格按模块拆分（模板/前端/报告），零冲突零覆盖，15 分钟完成 10 个 issue。关键是确保每个 agent 改的文件集合完全不重叠。

### 批量修复后的回归率（2026-04-17 新增）
- **10 个修复产生 4 个回归 bug（40% 回归率）** — #474/#475/#476/#477 都是第二轮修复引入的
- **回归根因：** 修复时的测试只验证了新场景，没覆盖旧数据兼容性（如旧报告无 dataStatus 字段）
- **经验：** 每轮修复后应跑全量 E2E 再宣布完成，不是只跑当次修复的测试
- **旧数据 + 新代码交叉测试** 是盲区 — TDD 测试用例应包含"旧数据格式输入"场景
...
### 空闲日不等于无事可做（2026-04-14 反思，04-21 三犯升级）
...
### 项目当前状态（2026-04-22 23:00 更新）
- **Open Issue = 0** 🎉 — 连续第五天保持归零（4/18-4/22）
- **系统健康：** 全 5 容器正常运行（frontend/backend Up 3d, postgres/redis/minio Up 11d）
- **全量 E2E 回归绿色：** Backend 371/371, Frontend 41/41
- **今日修复 5 个 issue (#535-#539)：** 测试密码过时、MetricsNormalizer mock、路由迁移遗留、异常检测空实现、过时单元测试
- **报告管线重构完成** — MetricsNormalizer + ScoringService 统一入口
- **Baseline 系统上线** — 规格匹配 + 覆盖率可视化 + 新鲜度告警 + 自动检测
- **⚠️ 待处理：** Baseline 设计文档 review、code review baseline 代码、测试覆盖率分析、设置 E2E 回归 weekly cron、HEARTBEAT.md 加空闲日主动工作规则
- **测试账号**: admin@ahvp.com / Test1234, test@ahvp.com / Test1234 (super_admin)

### 项目当前状态（旧 2026-04-17 23:00）
- **Open Issue = 0** 🎉 — 连续多日保持归零
- **今日关闭 15 个 issue**（#463-#477）— 模板系统+前端交互+报告生成+回归修复
- **模块隔离并行策略验证成功** — 3 agent 分模板/前端/报告并行，零冲突
- **CI 48 个 API 测试** — 需确认全绿
- **TDD 执行有进步** — 每个修复附带测试，39+6+7 = 52+ 新测试
- **回归率 40%** — 10 修复产生 4 回归，旧数据兼容性是主要盲区
- **待处理:** 15 个修复的浏览器验收、CI 健康检查、回归测试策略优化
- **测试账号**: admin@ahvp.com / Test1234, test@ahvp.com / Test1234 (super_admin)

### 项目当前状态（旧 2026-04-15 23:00）
...
- **模块隔离并行有效，但要预留回归窗口（2026-04-17）** — 3 个 agent 严格按模块（模板/前端/报告）拆分并行，零冲突。但 10 个修复产生 4 个回归（40%），说明每轮修复后应预留回归窗口而非直接宣布完成。
- **Agent timeout 先查产出再决定重跑（2026-04-17）** — 两个 agent 超时但代码修复+部署都已完成（超时在验证阶段）。主 session 看到 timeout 不能条件反射重跑，要先 `git log` + `docker ps` 确认实际产出。
- **回归 bug 本质是测试只覆盖新场景（2026-04-17）** — #476 回归是因旧报告无 dataStatus 字段，过滤逻辑未兼容。修复时的测试必须包含旧数据兼容性场景。
- **修复后回归率 40% 说明影响分析不够（2026-04-17）** — 10 个修复产生 4 个回归。每个修复前应花 1 分钟做 mini 影响分析：这个改动会影响哪些已有组件？旧数据格式是否兼容？关联组件是否需要同步改？不能只测新路径。
- **🔴🔴 findAll().stream() 在 @Transactional 中 = 定时炸弹（2026-04-18，第二次踩同类坑）** — JPA 一级缓存在事务内返回脏数据，#487 countFreeSlots 导致 16 个任务卡死。04-11 幽灵节点也是 JPA 缓存问题。**铁律：聚合查询（COUNT/SUM/AVG）一律用 @Query 注解 JPQL/SQL，禁止 findAll().stream().filter().count()。** 这是 code review 必检项。
- **Code Review 驱动开发 > Bug 驱动开发（2026-04-18）** — 两轮主动 code review（#488 调度 + #502 Agent）发现 44 个问题，其中多个 P0。不等 bug 上报而是主动审查代码质量，修复更系统、更彻底。产品经理做 code review 可以同时验证产品逻辑。
- **Plan 级 GPU 预留 vs Task 级分配（2026-04-18 产品设计教训）** — 区分"占用"和"使用"：方案选 gpu-4 就全预留 4 张，算子只用其中 1 张（CUDA_VISIBLE_DEVICES 控制），推理训练用全部。菜菜最初想降级为 gpu-1 是错误直觉，chenxi 纠正。产品层面的资源概念不能用技术思维简化。
- **API 回归测试是部署后质量底线（2026-04-18）** — 单元测试通过不代表集成正常。#487 JPA 脏读 bug 就是 API 回归测试发现的，比麦克雷报的 #485/#486 更严重。每次部署后必须跑完整 API 回归脚本。
- **双重 save + @Version 乐观锁 = 隐形地雷（2026-04-18）** — 同一事务内两次 `repository.save()` 触发 @Version 冲突。合并为单次 save，或将子操作拆到独立方法。code review 检查项。
- **Dispatch payload key 一致性要前置约定（2026-04-18）** — 后端 camelCase vs Agent snake_case 每次集成测试才发现。设计阶段应用 JSON Schema 约束接口契约，不要到联调再修。
...
- **E2E 测试必须定期清理，不能只做加法（2026-04-16）** — 444 个测试中 12 个是测不存在功能的骨架、5 个断言过时。旧测试不删 = 噪音掩盖信号。规则：功能大改后 review 相关 E2E，过时的删、断言更新、骨架不入 CI。
- **测试顺序依赖 = 隐形 bug（2026-04-16）** — 单独跑全绿但全量跑红，说明测试间有隐式依赖（共享 DB 状态、global-teardown 删数据）。每个测试必须 self-contained。
- **TDD 铁律执行差距（2026-04-16 chenxi 再次确认）** — #459 十个 phase 零测试暴露铁律写了但没执行。从今起 sub-agent task prompt 必须包含 TDD 三步结构，写入 docs/dev-standards.md 作为正式规范。铁律不落实到 prompt 模板 = 形同虚设。
- **CI timeout 应随测试增长主动调整（2026-04-16）** — 245 个测试 25 分钟 timeout 不够，每次都是跑挂了才发现。新增 >20 个测试时主动评估时间。
- **Build metadata 是可追溯性基础（2026-04-16）** — GIT_COMMIT/VERSION/BUILD_TIME 全部 unknown，线上问题无法定位代码版本。这是 Day 1 就应该有的基础设施。
- **Docker build args ≠ runtime env（2026-04-16）** — Dockerfile ARG 只在构建时有效，运行时需要 ENV。docker-compose.yml 的 build.args 和 environment 是两回事。版本信息要写到 .env 文件让 docker compose 在运行时传入。
- **E2E 测试分层：快测试 vs 慢测试（2026-04-16）** — API 测试 <1s/个适合每次 push 跑；执行流程测试需要 agent 真实处理 3min+/个，不能混在同一个 CI job 里。分层后 CI 从 40min 超时降到 2min40s。
- **🔴 系统性修复 > 自我承诺（2026-04-22，四天拖延血泪教训）** — 连续 4 天写"明天一定做"证明意志力驱动无效。主动工作触发必须写进 cron/heartbeat 规则，不靠"下次记得"。HEARTBEAT.md 已加 Step 2.5 空闲日主动工作规则。
- **E2E 回归应成为周期性任务（2026-04-22）** — 4 天没跑积累了 5 个问题（密码过期、路由遗留、断言漂移）。这些"隐形漂移"不会被用户报 bug，只有回归测试能抓到。应设置每周全量回归 cron。
- **启动是瓶颈，执行不是（2026-04-22）** — 31 分钟完成 5 个 fix 说明执行力够。问题 100% 在"什么时候开始"。降低启动摩擦（自动 spawn）比提高执行速度更有价值。
- **测试密码硬编码是定时炸弹（2026-04-22）** — #535 密码变更后 E2E/BDD 全量失败。测试凭据必须用环境变量管理，不要硬编码。已修复为 TEST_PASSWORD 环境变量。
...
### 开发冲刺经验（04-19 补充）
- **27 issue / 1 天 / 全自主** — 巡检发现 → 分析 → spawn → 验证 → 关闭，全天无需人工介入
- **Spring AOP self-invocation 陷阱** — 同 bean 内方法互调不走 proxy，@Transactional 不生效。#508 的根因，很常见
- **PyTorch DataParallel 小模型死锁** — scatter/gather 通信开销 > 计算时间时死锁。benchmark 场景跳过 DP
- **Chrome HTTPS-First + 443 端口 = 灾难** — 不用 HTTPS 就别映射 443，Chrome 会尝试 TLS 连接导致 broken fallback
- **重复 issue 快速识别** — 新 issue 进来先对比最近关闭的相同主题，避免重复劳动
- **Agent 自行关闭 issue OK** — 减少主 session 操作，巡检时确认即可
- **全自主模式边界意识（2026-04-19）** — 27 个 issue 零人工介入是效率新高，但大模块（如 Baseline 系统）跳过设计文档直接开干违反铁律。全自主 ≠ 不需要 review，新模块应主动找麦克雷 review
- **Baseline 等新模块需要先写设计文档（2026-04-19）** — 7 个 issue 的新模块直接开干虽然快，但缺少设计 review 可能埋坑。铁律：7+ issue 的新领域必须先出设计文档
- **27 issue/天全自主验证了模块隔离策略成熟度（2026-04-19）** — 报告管线(backend) + 任务 UX(full-stack) + baseline(backend) 三线并行零冲突，关键是文件集合不重叠
- **Spring AOP self-invocation 加入 code review checklist（2026-04-19）** — #508 再次验证：同 bean 内方法互调不走 proxy，@Transactional 不生效。这是 Java 开发最常见的坑之一，每次 review 必查

### 🔴 系统性修复 > 自我承诺（2026-04-22 四天教训总结）
- **现象：** 连续 4 天（4/19-4/22）写"明天一定做"的 lesson learned，连续 4 天白天空转不执行
- **根因：** 意志力驱动无效。心跳/cron 只检查"有没有坏"，不触发"有没有主动工作可做"
- **修复方向：** 把主动工作触发写进 cron/heartbeat（空闲日自动 spawn），不靠自我承诺
- **铁律：发现流程问题 → 当天修流程/工具，不写"明天改"**

### E2E 回归应周期化（2026-04-22）
- 4 天没跑积累了 5 个隐形问题（密码过期、路由遗留、断言漂移、空实现）
