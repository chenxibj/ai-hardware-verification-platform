...
- **🔴 需求必须先出设计文档（2026-04-19 铁律）** — 当 chenxi 提出"产品需求"/"需求"时，**必须先在 GitHub 项目 repo 里提交设计文档**（docs/ 目录），再拆 issue 再开发。流程：
...
### 项目当前状态（2026-04-18 23:00 更新）
- **Open Issue = 2**（#506 Agent 可观测性 P1 + #507 Agent 代码质量 P2，sub-agent 进行中）
- **今日关闭 30 个 issue**（#478-#505）— 单日新高
- **GPU 芯片级调度全栈完成** — #478 7 Phase + 麦克雷 review + 27 个衍生修复
- **两轮深度 Code Review** — #488 调度模块 27 问题 + #502 Agent 节点 17 问题
- **架构级重构落地** — TaskLifecycleService + PlanProgressService + GPU 枚举化 + 事务优化
- **安全加固** — Agent DB 直连移除 + Token 认证 + XSS 防护 + systemd 自愈
- **73+ 新 TDD 测试**，42 个 commit
- **待处理:** #506/#507 完成、全量 API 回归、浏览器验收、Plan 级 GPU 释放设计
