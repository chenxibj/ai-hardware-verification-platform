...
- **TDD 是开发核心方法论** —— 先写测试、再写实现、最后重构。不是"补测试"，是"测试驱动开发"。所有 issue 开发都按这个流程走。
...
- **验收要趁热打铁（2026-04-07）：** 晚间 11 个 issue 关闭后应立即启动验收而非等明天。Issue 刚关闭时上下文最清晰。
...
### 项目当前状态（2026-04-07 23:00 更新）
- **日志管理系统全栈开发完成**（P0+P1+P2）— #229-#234 代码已提交
  - WebSocket 实时推送、Agent 结构化上报、前端日志面板、搜索过滤、性能指标渲染、日志导出、保留策略
  - stats/metrics/report API 部署后 500 修复中（Docker 缓存问题）
  - 测试结果：22 passed / 8 skipped / 1 failed → 目标 ≥26 passed / 0 failed
- **分发引擎完成** — #222-#226 全部实现并关闭（端到端验证通过）
- **#206-#221 已验收通过**（16/16）
- **#75-#82 已关闭**（out of date）
- **今日关闭 11 个 issue**（#222-#226, #229-#234），仅剩 2 个 open（#227 #228）
- **待做：** #227 任务克隆 + #228 Web Terminal + API 500 修复 + 清数据给 chenxi 验证
- **Bug 已修：** 创建成功页乱码、"评测计划"→"评测任务"改名、模板详情弹窗 tags 报错、WebSocket 路径、API 格式
- **巡检机制升级** — active-tasks.json 追踪 + sub-agent 完成验证闭环
